...
if not('одинсубъект' or 'супружескаяпара' or 'родитель-одиночка' or 'родительодиночка' or 'один' or 'семья'
       or 'супруги'):
...
if subject_of_taxation == 'одинсубъект' or subject_of_taxation == 'один':
    sec, thi, fou, fif, sev = 9075, 36900, 89350, 186350, 406750
elif subject_of_taxation == 'супружескаяпара' or subject_of_taxation == 'супруги' or subject_of_taxation == 'семья':
...
if 0 <= annual_income <= sec:
    ...
    tax = 0.10 * sec + 0.15 * (thi - sec) + 0.25 * (fou - thi) + 0.28 * (fif - fou) + 0.33 * (annual_income - fif)
elif 405100 < annual_income <= sev:
    tax = 0.10 * sec + 0.15 * (thi - sec) + 0.25 * (fou - thi) + 0.28 * (fif - fou) + 0.33 * (405100 - fif) + 0.35 * \
          (annual_income - 405100)
elif sev < annual_income:
    tax = 0.10 * sec + 0.15 * (thi - sec) + 0.25 * (fou - thi) + 0.28 * (fif - fou) + 0.33 * (405100 - fif) + 0.35 * \
          (sev - 405100) + 0.396 * (annual_income - sev)
